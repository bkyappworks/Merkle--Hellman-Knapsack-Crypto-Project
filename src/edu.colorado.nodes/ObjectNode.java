package edu.colorado.nodes;// File: ObjectNode.java from the package edu.colorado.nodes
...
public class ObjectNode {
   ...
   /**
    * theta(n)
    * @precondition source is not a circular linked list.
    * Copy a list.
    * @param source
    *   the head of a linked list that will be copied (which may be
    *   an empty list in where source is null)*   @precondition
    * @return
   ...
   public static ObjectNode listCopy_rec(ObjectNode source) {
      ...
      copyTail = copyHead;
      // Make the rest of the nodes for the newly created list.
      listCopy_rec_helper(source,copyHead,copyTail);
      return copyHead.link;
   }

   private static ObjectNode listCopy_rec_helper(ObjectNode source, ObjectNode copyHead, ObjectNode copyTail) {
      if (source == null) {
         return null;
      }
//      System.out.println(source.data);
//      System.out.println("copyTail = "+copyTail);
      copyTail.addNodeAfter(source.data);
      return listCopy_rec_helper(source.link,copyHead,copyTail.link);
   }
   // listCopy_rec END
   ...
   public static int listLength_rec(ObjectNode head) {
      ...
      return listLength_rec(head,head,0);
   }

   private static int listLength_rec(ObjectNode head,ObjectNode cursor,int answer) {
      if (cursor == null) {
         return answer;
      }
      return listLength_rec(head, cursor.link,answer+1);
   }
   ...
   public static void main(String[] args) {
      // a. Create a list containing 26 nodes
      ObjectNode head = new ObjectNode("a",null);
      for (int i = 25; i >= 1; i--) {
         Object item = (char)(97 + i);
         head.addNodeAfter(item);
      }
      // b. Call toString() on the front node of the list to display the list data
      System.out.println(head.toString());
      // c. Call displayEveryThird()
      displayEveryThird(head);
      // d. Print the size of this list twice
      ...
      // i. On k2, call its toString() method
      System.out.println(k2.toString());
      // j. Print the size of the list k2 twice
